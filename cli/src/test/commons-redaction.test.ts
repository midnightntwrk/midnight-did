...
const fingerprintPrefix = (value: string): string => createHash('sha256').update(value).digest('hex').slice(0, 8);

describe('cli test commons redaction', () => {
  afterEach(() => {
    process.env = { ...ORIGINAL_ENV };
  });

  it('uses a seed fingerprint instead of raw seed characters in cache filenames', () => {
    const seed = 'not-a-real-secret-seed-for-cli-cache-tests';
    process.env.TEST_WALLET_SEED = seed;
    process.env.TEST_ENV = 'preview';

    const config = parseArgs(['seed', 'env']);

    expect(config.cacheFileName).toBe(`${fingerprintPrefix(seed)}-preview.state`);
