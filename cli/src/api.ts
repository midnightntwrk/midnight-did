...
export async function hashProverKey(proverKey: Uint8Array): Promise<Uint8Array> {
  ...
  return new Uint8Array(hash); // returns Uint8Array(32)
}

//TODO: It's a quick solution to use the prover key generated by the compactc. It should be discussed with the Midnight Team
export async function initPrivateState(providers: MidnightDIDProviders): Promise<MidnightDIDPrivateState> {
  ...
  const proverKey = await providers.zkConfigProvider.getProverKey('applyOperations');
  const proverKeyHash = await hashProverKey(proverKey);
  const privateState = {
    secretKey: proverKeyHash,
  };
  if (
    privateState != null &&
    ...
    logger.info('New state is fine...');
  else logger.info('Something is wrong with saved private state...');
