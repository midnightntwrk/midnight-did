...
| DID-PROD-103 | API transaction semantics | In progress | Make verification-method removal semantics explicit and resilient ([#240](https://github.com/midnightntwrk/midnight-did/issues/240)). | Default removal fails when the method is referenced, or purge-then-remove is exposed as an explicit advanced operation with recovery/idempotency guidance. |
