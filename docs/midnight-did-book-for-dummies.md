...
|---|---|---|---|---|---|
| VC profile in `docs/midnight-did-use-cases.md` (`Recommended compact-friendly profile` section) | High | Red | No single canonical VC schema is adopted by the repository. | Blocks deterministic signing/verification and cross-service reuse. | One schema file exists under `prototypes/` (or equivalent agreed path) and both issuer/verifier sample flows pass deterministic hash tests. |
