...
- **Sign and sign off every commit:** Every PR commit, including automation-authored commits, must have a GitHub-verified GPG signature and a terminal DCO trailer matching the commit author (`Signed-off-by: Name <email>`). SSH signatures and bot DCO exemptions are not accepted.
