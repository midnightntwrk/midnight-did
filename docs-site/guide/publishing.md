...
| --- | --- | --- | --- | --- | --- |
| Snapshot | Push | `main` | `x.y.z-snapshot.<run>.<sha>` | `snapshot` | Workflow artifact and GHCR OCI artifact |
...
pushes the bundle to GHCR, pulls it back, and validates it again. RC and release
runs also upload the bundle to a GitHub Release and download the asset back for
the same validation.
