...
vc-jws-2020 context. Midnight emits `JsonWebKey` verification methods with
`publicKeyJwk` material for all supported key profiles; it does not emit
`JsonWebKey2020`. Consumers that require `JsonWebKey2020` naming should adapt at
their integration boundary rather than changing the canonical Midnight DID
Document.
