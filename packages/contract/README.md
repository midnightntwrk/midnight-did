...
sequenceDiagram
  ...
  Circuit->>Circuit: verify controller signature over contract id + current version
...
- Controller public keys can be derived locally with `deriveControllerPublicKey(secretKey)` as Jubjub points. Controller-gated circuits verify wallet-local signatures over a domain-separated digest containing the DID contract id and current version.
