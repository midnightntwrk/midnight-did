...
import { LedgerToDomain, MidnightNetwork, parseContractAddress, } from "../index.js";
...
describe("LedgerToDomain (unit, mocked managed runtime)", () => {
    ...
    const keyString = (fill) => Buffer.from(bytes32(fill)).toString("base64url");
    ...
    const fieldString = (value) => Buffer.from(bigintTo32Le(value)).toString("base64url");
    let stubLedger;
    beforeEach(() => {
        ...
        stubLedger = {
            ...
            verificationMethods,
            schnorrJubjubVerificationMethods: makeIterablePairs([]),
    ...
    it("publicKeyJwk rejects malformed opaque string values", () => {
        expect(() => LedgerToDomain.publicKeyJwk({
            kty: DIDContract.KeyType.OKP,
            crv: DIDContract.CurveType.Ed25519,
            x: "not-base64url!",
            y: "",
    ...
    it("toJSON flattens ledger to plain JSON with arrays", () => {
        ...
        expect(json.verificationMethods[0].publicKeyJwk.x).toBe("AQEBAQEBAQEBAQEBAQEBAQEBAQEBAQEBAQEBAQEBAQE");
    ...
    it("ledgerStateToDIDDocument merges native SchnorrJubjub methods", () => {
        ...
        stubLedger.assertionMethodRelation = makeIterable([
            "key-schnorr-jubjub",
        ]);
        const doc = LedgerToDomain.ledgerStateToDIDDocument(stubLedger, MidnightNetwork.DevNet, addr);
        expect(doc.verificationMethod?.length).toBe(2);
        const nativeMethod = doc.verificationMethod?.find((method) => method.id.endsWith("#key-schnorr-jubjub"));
    ...
    it("ledgerStateToDIDDocument rejects duplicate normalized verification method ids", () => {
        ...
        expect(() => LedgerToDomain.ledgerStateToDIDDocument(stubLedger, MidnightNetwork.DevNet, addr)).toThrow(/Duplicate verification method id/);
    });
