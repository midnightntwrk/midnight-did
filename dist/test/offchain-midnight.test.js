...
import { createLongFormOffchainMidnightDIDString, createOffchainMidnightDIDStringFromState, decodeOffchainMidnightDIDState, encodeOffchainMidnightDIDState, offchainStateToDidDocument, parseLongFormOffchainMidnightDIDString, } from "../offchain-midnight.js";
const sampleState = {
    version: 1,
...
describe("offchain Midnight DID helpers", () => {
    ...
    it("creates and parses a self-contained long-form offchain Midnight DID", () => {
        ...
        expect(parsed.did).toBe(createLongFormOffchainMidnightDIDString(sampleState));
    ...
    it("rejects the hash-only short form when encoded state is required", () => {
        expect(() => parseLongFormOffchainMidnightDIDString(createOffchainMidnightDIDStringFromState(sampleState))).toThrow(/must include encoded state/);
    });
    it("rejects the old DID URL query state shape", () => {
        ...
        expect(() => parseLongFormOffchainMidnightDIDString(`${shortForm}?state=${encoded.payload}`)).toThrow(/Invalid method-specific identifier/);
    });
    it("rejects empty long-form encoded state", () => {
        const shortForm = createOffchainMidnightDIDStringFromState(sampleState);
        expect(() => parseLongFormOffchainMidnightDIDString(`${shortForm}:`)).toThrow(/state encoding/);
    });
    it("rejects non-base64url long-form state before hash validation", () => {
        const shortForm = createOffchainMidnightDIDStringFromState(sampleState);
        expect(() => parseLongFormOffchainMidnightDIDString(`${shortForm}:not+base64url`)).toThrow(/state encoding/);
    });
    it("rejects non-canonical base64url long-form state", () => {
        ...
        expect(fromBase64UrlUnsafe(variant)).toEqual(fromBase64UrlUnsafe(parts[4] ?? ""));
        expect(() => parseLongFormOffchainMidnightDIDString(`${parts.slice(0, 4).join(":")}:${variant}`)).toThrow(/canonical unpadded base64url/);
    });
    it("rejects a long-form DID when the state payload is tampered", () => {
        ...
        expect(() => parseLongFormOffchainMidnightDIDString(tampered)).toThrow(/state does not match the DID state hash/);
    });
