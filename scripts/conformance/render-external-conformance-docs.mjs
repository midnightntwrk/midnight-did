...
The supplementary harness is [\`w3c/did-test-suite\`](${baseline.upstream.repository}) at commit \`${baseline.upstream.commit}\` (archive SHA-256 \`${baseline.upstream.archiveSha256}\`; complete extracted-tree SHA-256 \`${baseline.upstream.extractedTreeSha256}\`). It requires Node major ${baseline.toolchains.nodeMajor} and npm major ${baseline.toolchains.npmMajor}, while canonical evidence records the exact accepted Node and npm versions used. It runs only these suites, with pinned expected assertion counts: ${selectedSuiteSummary}. They execute against a deterministic descriptor generated through the built public \`@midnight-ntwrk/midnight-did\` exports. \`did-consumption\` is labelled representation consumability because generation first round-trips the JSON-LD representation through \`parseMidnightDIDDocument\`; it is not broader semantic conformance.
