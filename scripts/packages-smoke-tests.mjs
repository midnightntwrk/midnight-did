  ...
  try {
    ...
    await writeFile(
      ...
      `
        ...
        if (api.MIDNIGHT_DID_API_VERSION !== "0.6.0") {
        ...
        if (locations.ghcr.reference !== "ghcr.io/midnightntwrk/midnight-did-zk-artifacts:0.6.0") {
        ...
        if (apiBrowser.MIDNIGHT_DID_ZK_ARTIFACT_LOCATIONS.version !== "0.6.0") {
  ...
  try {
    ...
    await writeFile(
      ...
      `
        ...
        if (!DomainToRuntime.NetworkMap || !RuntimeToDomain.NetworkMap || parsedSeed.length !== 64 || random.length !== 4 || MIDNIGHT_DID_ZK_ARTIFACT_LOCATIONS.version !== "0.6.0") {
