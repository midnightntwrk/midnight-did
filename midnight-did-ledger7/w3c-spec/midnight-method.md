...
Updating the Midnight DID implies that the DID Controller calls one of the smart contract's 11 individual circuits for each type of modification.
...
The circuit implementations are in the [did.compact](#) file, and the API functions that call these circuits are in the [api.ts](#) file.
...
Example (Ed25519):
```typescript
await addVerificationMethod(didContract, '#key-1', {
  kty: 'OKP',
  crv: 'Ed25519',
  x: 42n,
  y: 0n
...
Example:
```typescript
await updateVerificationMethod(didContract, '#key-1', {
  kty: 'OKP',
  crv: 'Ed25519',
  x: 8n,
  y: 0n
...
Example:
```typescript
await removeVerificationMethod(didContract, '#key-1');
```
...
Example:
```typescript
await addService(didContract, '#didcomm-1', 'DIDCommV2', 'https://localhost/didcomm/v2');
```
...
Example:
```typescript
await updateService(didContract, '#didcomm-1', 'DIDCommV2', 'https://new-endpoint.com/didcomm');
```
