...
- If submission or finality throws before returning finalized transaction data, the helper MUST retain pending state because a thrown receipt/finality stream cannot distinguish rejection from successful on-chain finalization. The application MUST re-read the on-ledger `controllerPublicKey` before retrying. If the replacement key is active, it can promote the pending state with the public recovery helper by passing the providers and `{ contractAddress, rotationFinalized: true }`; if the old key remains active, it MUST discard the stale candidate explicitly with the public discard helper and `{ contractAddress, rotationFinalized: false }` before retrying.
- While pending controller state exists, a later rotation or recovery MUST fail before authorization or a transaction-call attempt with <code>PendingControllerPrivateState<wbr>ExistsError</code> (<code>pending_controller_private_state_<wbr>exists</code>) instead of replacing that state. A rotation, recovery, promotion, or discard racing an in-flight pending-state lifecycle MUST fail with <code>PendingControllerPrivateState<wbr>BusyError</code> (<code>pending_controller_private_state_<wbr>busy</code>) and MUST NOT write active state or remove that lifecycle's candidate. The critical section begins before the first provider-dependent active/recovery-state or ledger preflight and spans the pending-slot absence check and persistence, authorization, transaction-call attempt, active promotion, and pending cleanup.
- `bindPrivateStateProvider` keys that process-local critical section by canonical contract address, serializing different provider wrappers bound through the API to the same DID. Rotation, recovery, and reconciliation MUST bind an API-untracked provider to the expected canonical address and reject a known different API-tracked binding with <code>PrivateStateProviderContract<wbr>MismatchError</code> (<code>private_state_provider_contract_<wbr>mismatch</code>) before provider, ledger, authorization, or transaction access. Deploy MUST hold lease-owned source and returned-target reservations through active private-state persistence. While the provider's current key or the requested canonical address key is reserved, `bindPrivateStateProvider` (and therefore `joinContract`) MUST fail closed with <code>PendingControllerPrivateState<wbr>BusyError</code> before mutating the provider address or lock-key mapping; this includes same-address rebinding during preflight and the rest of the lifecycle. Direct `privateStateProvider.setContractAddress` calls bypass API coordination and MUST NOT occur during rotation, recovery, promotion/discard reconciliation, or pending cleanup. Explicitly unbound custom providers fall back to wrapper identity. The provider interface has no cross-process compare-and-set primitive; applications using separate processes or independently unbound wrappers against one DID private-state store MUST enforce an external per-DID single-writer lock. This process-local mechanism does not protect direct provider mutation and does not provide cross-process CAS.
