...
- While pending controller state exists, another rotation or recovery MUST fail before authorization/submission with the typed pending-state error (`code: "pendingControllerPrivateStateExists"`) instead of replacing that state. Calls sharing one provider object are serialized around the pending-slot absence check. Because the provider interface has no cross-process compare-and-set primitive, applications with multiple provider instances or processes writing one DID private-state store MUST enforce an external single-writer lock.
