...
- If the transaction finalizes but active private-state persistence fails, the wallet must recover the same pending secret to continue updating the DID. If explicit pending promotion writes active state but pending cleanup fails, it MUST warn and return the promoted state while retaining the candidate for a safe idempotent reconciliation retry.
